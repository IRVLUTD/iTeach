...
def unpack_packet(data):
    ...
    return _packet(
        timestamp,
        payload,
        np.frombuffer(pose, dtype=np.float32).reshape((4, 4))
        if (len(pose) == 64)
        else None,
    )
            ...
            if self._state == 0:
                ...
                if length >= self._size:
                    if self._mode == StreamMode.MODE_1:
                        ...
                        self._pose = np.frombuffer(
                            self._buffer[payload_end : self._size], dtype=np.float32
                        ).reshape((4, 4))
                    else:
...
def _create_configuration_for_mrc(
    enable,
    hologram_composition,
    recording_indicator,
    video_stabilization,
    blank_protected,
    show_mesh,
    ...
    output_width,
    output_height,
    video_stabilization_length,
    hologram_perspective,
):
    return struct.pack(
        "<BBBBBBfffII",
        1 if (enable) else 0,
        1 if (hologram_composition) else 0,
        1 if (recording_indicator) else 0,
        1 if (video_stabilization) else 0,
        1 if (blank_protected) else 0,
        1 if (show_mesh) else 0,
        global_opacity,
        output_width,
        output_height,
        video_stabilization_length,
...
def _create_configuration_for_rm_vlc(mode, divisor, profile, level, bitrate, options):
    ...
    configuration.extend(
        _create_configuration_for_video_encoding(profile, level, bitrate)
    )
    configuration.extend(_create_configuration_for_h26x_encoding(options))
    return bytes(configuration)


def _create_configuration_for_rm_depth_ahat(
    mode, divisor, profile_z, profile_ab, level, bitrate, options
):
    configuration = bytearray()
    configuration.extend(_create_configuration_for_mode(mode))
    configuration.extend(_create_configuration_for_video_divisor(divisor))
    ...
    configuration.extend(
        _create_configuration_for_video_encoding(profile_ab, level, bitrate)
    )
    configuration.extend(_create_configuration_for_h26x_encoding(options))
    return bytes(configuration)
...
def _create_configuration_for_pv(
    mode, width, height, framerate, divisor, profile, level, bitrate, options
):
    configuration = bytearray()
    configuration.extend(_create_configuration_for_mode(mode))
    configuration.extend(
        _create_configuration_for_video_format(width, height, framerate)
    )
    configuration.extend(_create_configuration_for_video_divisor(divisor))
    configuration.extend(
        _create_configuration_for_video_encoding(profile, level, bitrate)
...
def _connect_client_rm_vlc(
    host, port, chunk_size, mode, divisor, profile, level, bitrate, options
):
    c = _gatherer()
    c.open(host, port, chunk_size, mode)
    c.sendall(
        _create_configuration_for_rm_vlc(
            mode, divisor, profile, level, bitrate, options
        )
    )
    return c


def _connect_client_rm_depth_ahat(
    host,
    port,
    chunk_size,
    mode,
    ...
    c.open(host, port, chunk_size, mode)
    c.sendall(
        _create_configuration_for_rm_depth_ahat(
            mode, divisor, profile_z, profile_ab, level, bitrate, options
        )
    )
    return c
...
def _connect_client_rm_depth_longthrow(
    host, port, chunk_size, mode, divisor, png_filter
):
    c = _gatherer()
    c.open(host, port, chunk_size, mode)
...
def start_subsystem_pv(
    ...
    c.open(host, port)
    c.sendall(
        _create_configuration_for_pv_mode2(_PVCNT.START | _PVCNT.MODE_3, 1920, 1080, 30)
    )
    c.sendall(
        _create_configuration_for_mrc(
            enable_mrc,
            hologram_composition,
            recording_indicator,
            video_stabilization,
...
def stop_subsystem_pv(host, port):
    ...
    c.open(host, port)
    c.sendall(
        _create_configuration_for_pv_mode2(_PVCNT.STOP | _PVCNT.MODE_3, 1920, 1080, 30)
    )
    c.close()
...
class rx_rm_vlc(_context_manager):
    def __init__(
        self, host, port, chunk, mode, divisor, profile, level, bitrate, options
    ):
        self.host = host
        self.port = port
        self.chunk = chunk
    ...
    def open(self):
        self._client = _connect_client_rm_vlc(
            self.host,
            self.port,
            self.chunk,
            self.mode,
...
class rx_rm_depth_ahat(_context_manager):
    ...
    def open(self):
        self._client = _connect_client_rm_depth_ahat(
            self.host,
            self.port,
            self.chunk,
            self.mode,
...
class rx_rm_depth_longthrow(_context_manager):
    ...
    def open(self):
        self._client = _connect_client_rm_depth_longthrow(
            self.host, self.port, self.chunk, self.mode, self.divisor, self.png_filter
        )
...
class rx_rm_imu(_context_manager):
    ...
    def open(self):
        self._client = _connect_client_rm_imu(
            self.host, self.port, self.chunk, self.mode
        )
...
class rx_pv(_context_manager):
    ...
    def open(self):
        self._client = _connect_client_pv(
            self.host,
            self.port,
            self.chunk,
            self.mode,
...
class rx_microphone(_context_manager):
    ...
    def open(self):
        self._client = _connect_client_microphone(
            self.host, self.port, self.chunk, self.profile, self.level
        )
        ...
        for packet in self._codec.parse(payload):
            for frame in self._codec.decode(packet):
                return frame.to_ndarray()[
                    : Parameters_RM_VLC.HEIGHT, : Parameters_RM_VLC.WIDTH
                ]
        return None
...
def decode_rm_vlc(profile):
    return (
        _unpack_rm_vlc() if (profile == VideoProfile.RAW) else _decode_rm_vlc(profile)
    )
...
def _unpack_rm_depth_ahat_nv12_as_yuv420p(yuv):
    y = yuv[
        _Mode0Layout_RM_DEPTH_AHAT.BEGIN_DEPTH_Y : _Mode0Layout_RM_DEPTH_AHAT.END_DEPTH_Y,
        :,
    ]
    u = yuv[
        _Mode0Layout_RM_DEPTH_AHAT.BEGIN_AB_U_Y : _Mode0Layout_RM_DEPTH_AHAT.END_AB_U_Y,
        :,
    ].reshape((Parameters_RM_DEPTH_AHAT.HEIGHT, Parameters_RM_DEPTH_AHAT.WIDTH // 4))
    v = yuv[
        _Mode0Layout_RM_DEPTH_AHAT.BEGIN_AB_V_Y : _Mode0Layout_RM_DEPTH_AHAT.END_AB_V_Y,
        :,
    ].reshape((Parameters_RM_DEPTH_AHAT.HEIGHT, Parameters_RM_DEPTH_AHAT.WIDTH // 4))

    depth = np.multiply(y, 4, dtype=np.uint16)
    ab = np.empty(
        (Parameters_RM_DEPTH_AHAT.HEIGHT, Parameters_RM_DEPTH_AHAT.WIDTH),
        dtype=np.uint16,
    )
...
class _unpack_rm_depth_ahat:
    ...
    def decode(self, payload):
        depth = np.frombuffer(
            payload, dtype=np.uint16, offset=0, count=Parameters_RM_DEPTH_AHAT.PIXELS
        ).reshape(Parameters_RM_DEPTH_AHAT.SHAPE)
        ab = np.frombuffer(
            payload,
            dtype=np.uint16,
            offset=Parameters_RM_DEPTH_AHAT.PIXELS * _SIZEOF.WORD,
            count=Parameters_RM_DEPTH_AHAT.PIXELS,
        ).reshape(Parameters_RM_DEPTH_AHAT.SHAPE)
        depth[depth >= 4090] = 0
        return _RM_Depth_Frame(depth, ab)
    ...
    def decode(self, payload):
        for packet in self._codec.parse(payload):
            for frame in self._codec.decode(packet):
                return np.square(
                    frame.to_ndarray()[
                        : Parameters_RM_DEPTH_AHAT.HEIGHT,
                        : Parameters_RM_DEPTH_AHAT.WIDTH,
                    ],
                    dtype=np.uint16,
                )
        return None
...
class _decode_rm_depth_ahat_zdepth:
    def __init__(self, profile):
        ...
        self._codec_ab = (
            _unpack_ab_rm_depth_ahat()
            if (profile == VideoProfile.RAW)
            else _decode_ab_rm_depth_ahat(profile)
        )
...
def decode_rm_depth_ahat(profile_z, profile_ab):
    return (
        (
            _unpack_rm_depth_ahat()
            if (profile_ab == VideoProfile.RAW)
            else _decode_rm_depth_ahat(profile_ab)
        )
        if (profile_z == DepthProfile.SAME)
        else _decode_rm_depth_ahat_zdepth(profile_ab)
    )
...
def unpack_pv(payload):
    return _PV_Frame(
        payload[:-16],
        np.frombuffer(payload[-16:-8], dtype=np.float32),
        np.frombuffer(payload[-8:], dtype=np.float32),
    )
...
class _unpack_pv:
    ...
    def decode(self, payload, format):
        image = np.frombuffer(payload, dtype=np.uint8).reshape(
            (int(self.height * 3 / 2), self.stride)
        )[:, : self.width]
...
def decode_microphone(profile):
    return (
        _unpack_microphone()
        if (profile == AudioProfile.RAW)
        else _decode_microphone(profile)
    )
...
class _Mode0Layout_SI:
    ...
    END_HAND_LEFT = (
        BEGIN_HAND_LEFT + SI_HandJointKind.TOTAL * _Mode0Layout_SI_Hand.BYTE_COUNT
    )
    BEGIN_HAND_RIGHT = END_HAND_LEFT
    END_HAND_RIGHT = (
        BEGIN_HAND_RIGHT + SI_HandJointKind.TOTAL * _Mode0Layout_SI_Hand.BYTE_COUNT
    )
...
class _SI_Hand:
    ...
    def get_joint_pose(self, joint):
        ...
        orientation = np.frombuffer(
            data[
                _Mode0Layout_SI_Hand.BEGIN_ORIENTATION : _Mode0Layout_SI_Hand.END_ORIENTATION
            ],
            dtype=np.float32,
        )
        position = np.frombuffer(
            data[
                _Mode0Layout_SI_Hand.BEGIN_POSITION : _Mode0Layout_SI_Hand.END_POSITION
            ],
            dtype=np.float32,
        )
        radius = np.frombuffer(
            data[_Mode0Layout_SI_Hand.BEGIN_RADIUS : _Mode0Layout_SI_Hand.END_RADIUS],
            dtype=np.float32,
        )
        accuracy = np.frombuffer(
            data[
                _Mode0Layout_SI_Hand.BEGIN_ACCURACY : _Mode0Layout_SI_Hand.END_ACCURACY
            ],
            dtype=np.int32,
        )
...
class unpack_si:
    def __init__(self, payload):
        ...
        self._valid = np.frombuffer(
            payload[_Mode0Layout_SI.BEGIN_VALID : _Mode0Layout_SI.END_VALID],
            dtype=np.uint8,
        )
    ...
    def get_head_pose(self):
        position = np.frombuffer(
            self._data[
                _Mode0Layout_SI.BEGIN_HEAD_POSITION : _Mode0Layout_SI.END_HEAD_POSITION
            ],
            dtype=np.float32,
        )
        forward = np.frombuffer(
            self._data[
                _Mode0Layout_SI.BEGIN_HEAD_FORWARD : _Mode0Layout_SI.END_HEAD_FORWARD
            ],
            dtype=np.float32,
        )
        up = np.frombuffer(
            self._data[_Mode0Layout_SI.BEGIN_HEAD_UP : _Mode0Layout_SI.END_HEAD_UP],
            dtype=np.float32,
        )
    ...
    def get_eye_ray(self):
        origin = np.frombuffer(
            self._data[
                _Mode0Layout_SI.BEGIN_EYE_ORIGIN : _Mode0Layout_SI.END_EYE_ORIGIN
            ],
            dtype=np.float32,
        )
        direction = np.frombuffer(
            self._data[
                _Mode0Layout_SI.BEGIN_EYE_DIRECTION : _Mode0Layout_SI.END_EYE_DIRECTION
            ],
            dtype=np.float32,
        )
    ...
    def get_hand_left(self):
        return _SI_Hand(
            self._data[_Mode0Layout_SI.BEGIN_HAND_LEFT : _Mode0Layout_SI.END_HAND_LEFT]
        )

    def get_hand_right(self):
        return _SI_Hand(
            self._data[
                _Mode0Layout_SI.BEGIN_HAND_RIGHT : _Mode0Layout_SI.END_HAND_RIGHT
            ]
        )
...
class rx_decoded_rm_vlc(rx_rm_vlc):
    def __init__(
        ...
        super().__init__(
            host, port, chunk, mode, divisor, profile, level, bitrate, options
        )
...
def download_calibration_rm_vlc(host, port):
    data = _download_mode2_data(
        host,
        port,
        _create_configuration_for_rm_mode2(StreamMode.MODE_2),
        _Mode2Layout_RM_VLC.FLOAT_COUNT * _SIZEOF.FLOAT,
    )
    floats = np.frombuffer(data, dtype=np.float32)

    uv2x = floats[
        _Mode2Layout_RM_VLC.BEGIN_UV2X : _Mode2Layout_RM_VLC.END_UV2X
    ].reshape(Parameters_RM_VLC.SHAPE)
    uv2y = floats[
        _Mode2Layout_RM_VLC.BEGIN_UV2Y : _Mode2Layout_RM_VLC.END_UV2Y
    ].reshape(Parameters_RM_VLC.SHAPE)
    extrinsics = floats[
        _Mode2Layout_RM_VLC.BEGIN_EXTRINSICS : _Mode2Layout_RM_VLC.END_EXTRINSICS
    ].reshape((4, 4))
    mapx = floats[
        _Mode2Layout_RM_VLC.BEGIN_MAPX : _Mode2Layout_RM_VLC.END_MAPX
    ].reshape(Parameters_RM_VLC.SHAPE)
    mapy = floats[
        _Mode2Layout_RM_VLC.BEGIN_MAPY : _Mode2Layout_RM_VLC.END_MAPY
    ].reshape(Parameters_RM_VLC.SHAPE)
    k = floats[_Mode2Layout_RM_VLC.BEGIN_K : _Mode2Layout_RM_VLC.END_K]

    intrinsics = np.array(
        [[k[0], 0, 0, 0], [0, k[1], 0, 0], [k[2], k[3], 1, 0], [0, 0, 0, 1]],
        dtype=np.float32,
    )

    return _Mode2_RM_VLC(
        np.dstack((uv2x, uv2y)), extrinsics, np.dstack((mapx, mapy)), intrinsics
    )
...
def download_calibration_rm_depth_ahat(host, port):
    ...
    uv2x = floats[
        _Mode2Layout_RM_DEPTH_AHAT.BEGIN_UV2X : _Mode2Layout_RM_DEPTH_AHAT.END_UV2X
    ].reshape(Parameters_RM_DEPTH_AHAT.SHAPE)
    uv2y = floats[
        _Mode2Layout_RM_DEPTH_AHAT.BEGIN_UV2Y : _Mode2Layout_RM_DEPTH_AHAT.END_UV2Y
    ].reshape(Parameters_RM_DEPTH_AHAT.SHAPE)
    extrinsics = floats[
        _Mode2Layout_RM_DEPTH_AHAT.BEGIN_EXTRINSICS : _Mode2Layout_RM_DEPTH_AHAT.END_EXTRINSICS
    ].reshape((4, 4))
    scale = floats[
        _Mode2Layout_RM_DEPTH_AHAT.BEGIN_SCALE : _Mode2Layout_RM_DEPTH_AHAT.END_SCALE
    ]
    alias = floats[
        _Mode2Layout_RM_DEPTH_AHAT.BEGIN_ALIAS : _Mode2Layout_RM_DEPTH_AHAT.END_ALIAS
    ]
    mapx = floats[
        _Mode2Layout_RM_DEPTH_AHAT.BEGIN_MAPX : _Mode2Layout_RM_DEPTH_AHAT.END_MAPX
    ].reshape(Parameters_RM_DEPTH_AHAT.SHAPE)
    mapy = floats[
        _Mode2Layout_RM_DEPTH_AHAT.BEGIN_MAPY : _Mode2Layout_RM_DEPTH_AHAT.END_MAPY
    ].reshape(Parameters_RM_DEPTH_AHAT.SHAPE)
...
def download_calibration_rm_depth_longthrow(host, port):
    ...
    uv2x = floats[
        _Mode2Layout_RM_DEPTH_LONGTHROW.BEGIN_UV2X : _Mode2Layout_RM_DEPTH_LONGTHROW.END_UV2X
    ].reshape(Parameters_RM_DEPTH_LONGTHROW.SHAPE)
    uv2y = floats[
        _Mode2Layout_RM_DEPTH_LONGTHROW.BEGIN_UV2Y : _Mode2Layout_RM_DEPTH_LONGTHROW.END_UV2Y
    ].reshape(Parameters_RM_DEPTH_LONGTHROW.SHAPE)
    extrinsics = floats[
        _Mode2Layout_RM_DEPTH_LONGTHROW.BEGIN_EXTRINSICS : _Mode2Layout_RM_DEPTH_LONGTHROW.END_EXTRINSICS
    ].reshape((4, 4))
    scale = floats[
        _Mode2Layout_RM_DEPTH_LONGTHROW.BEGIN_SCALE : _Mode2Layout_RM_DEPTH_LONGTHROW.END_SCALE
    ]
    mapx = floats[
        _Mode2Layout_RM_DEPTH_LONGTHROW.BEGIN_MAPX : _Mode2Layout_RM_DEPTH_LONGTHROW.END_MAPX
    ].reshape(Parameters_RM_DEPTH_LONGTHROW.SHAPE)
    mapy = floats[
        _Mode2Layout_RM_DEPTH_LONGTHROW.BEGIN_MAPY : _Mode2Layout_RM_DEPTH_LONGTHROW.END_MAPY
    ].reshape(Parameters_RM_DEPTH_LONGTHROW.SHAPE)
    k = floats[
        _Mode2Layout_RM_DEPTH_LONGTHROW.BEGIN_K : _Mode2Layout_RM_DEPTH_LONGTHROW.END_K
    ]
    ...
    return _Mode2_RM_DEPTH_LONGTHROW(
        np.dstack((uv2x, uv2y)), extrinsics, scale, np.dstack((mapx, mapy)), intrinsics
    )
...
def download_calibration_rm_imu(host, port):
    ...
    extrinsics = floats[
        _Mode2Layout_RM_IMU.BEGIN_EXTRINSICS : _Mode2Layout_RM_IMU.END_EXTRINSICS
    ].reshape((4, 4))
...
def download_calibration_pv(host, port, width, height, framerate):
    ...
    focal_length = floats[
        _Mode2Layout_PV.BEGIN_FOCALLENGTH : _Mode2Layout_PV.END_FOCALLENGTH
    ]
    principal_point = floats[
        _Mode2Layout_PV.BEGIN_PRINCIPALPOINT : _Mode2Layout_PV.END_PRINCIPAL_POINT
    ]
    radial_distortion = floats[
        _Mode2Layout_PV.BEGIN_RADIALDISTORTION : _Mode2Layout_PV.END_RADIALDISTORTION
    ]
    tangential_distortion = floats[
        _Mode2Layout_PV.BEGIN_TANGENTIALDISTORTION : _Mode2Layout_PV.END_TANGENTIALDISTORTION
    ]
    projection = floats[
        _Mode2Layout_PV.BEGIN_PROJECTION : _Mode2Layout_PV.END_PROJECTION
    ].reshape((4, 4))
...
class ipc_rc(_context_manager):
    ...
    def set_pv_exposure_priority_video(self, enabled):
        command = struct.pack(
            "<BI", ipc_rc._CMD_SET_PV_EXPOSURE_PRIORITY_VIDEO, enabled
        )
        self._client.sendall(command)
...
class sm_bounding_volume:
    ...
    def add_sphere(self, center, radius):
        self._add(
            struct.pack(
                "<Iffff", _SM_VolumeType.Sphere, center[0], center[1], center[2], radius
            )
        )
...
class sm_mesh_task:
    ...
    def add_task(
        self,
        id,
        max_triangles_per_cubic_meter,
        vertex_position_format,
        triangle_index_format,
        vertex_normal_format,
        include_vertex_normals,
        include_bounds,
    ):
        self._data.extend(
            struct.pack(
                ...
                triangle_index_format,
                vertex_normal_format,
                (1 if include_vertex_normals else 0) | (2 if include_bounds else 0),
            )
        )
        self._count += 1
...
class _sm_mesh:
    ...
    def unpack(
        self, vertex_position_format, triangle_index_format, vertex_normal_format
    ):
        self.vertex_position_scale = np.frombuffer(
            self.vertex_position_scale, dtype=np.float32
        ).reshape((1, 3))
        self.pose = np.frombuffer(self.pose, dtype=np.float32).reshape((4, 4))
        self.bounds = np.frombuffer(self.bounds, dtype=np.float32)
        self.vertex_positions = np.frombuffer(
            self.vertex_positions,
            dtype=_SM_Convert.DirectXPixelFormatToNumPy[vertex_position_format],
        ).reshape((-1, 4))
        self.triangle_indices = np.frombuffer(
            self.triangle_indices,
            dtype=_SM_Convert.DirectXPixelFormatToNumPy[triangle_index_format],
        ).reshape((-1, 3))
        self.vertex_normals = np.frombuffer(
            self.vertex_normals,
            dtype=_SM_Convert.DirectXPixelFormatToNumPy[vertex_normal_format],
        ).reshape((-1, 4))
...
class ipc_sm(_context_manager):
    ...
    def get_observed_surfaces(self):
        ...
        count = struct.unpack(
            "<Q", self._client.download(_SIZEOF.QWORD, ChunkSize.SINGLE_TRANSFER)
        )[0]
        ids = self._client.download(count * 24, ChunkSize.SINGLE_TRANSFER)
        return [
            _sm_surface_info(
                ids[(i * 24) : (i * 24 + 16)],
                struct.unpack("<Q", ids[(i * 24 + 16) : (i * 24 + 24)])[0],
            )
    ...
    def _download_mesh(self):
        ...
        payload = self._client.download(
            bsz + vpl + til + vnl, ChunkSize.SINGLE_TRANSFER
        )
        ...
        return index, _sm_mesh(
            scale, pose, bounds, vertex_positions, triangle_indices, vertex_normals
        )
    ...
    def pack(self):
        ...
        self._task.extend(
            struct.pack(
                ...
                self.enable_quads,
                self.enable_meshes,
                self.enable_only_observed,
                self.enable_world_mesh,
                self.mesh_lod,
                self.query_radius,
                self.create_mode,
                self.kind_flags,
                ...
                self.get_position,
                self.get_location_matrix,
                self.get_quad,
                self.get_meshes,
                self.get_collider_meshes,
                len(self.guid_list),
            )
        )
...
class _su_mesh:
    ...
    def unpack(self):
        self.vertex_positions = np.frombuffer(
            self.vertex_positions, dtype=np.float32
        ).reshape((-1, 3))
        self.triangle_indices = np.frombuffer(
            self.triangle_indices, dtype=np.uint32
        ).reshape((-1, 3))
...
class _su_result:
    ...
    def unpack(self):
        self.extrinsics = np.frombuffer(self.extrinsics, dtype=np.float32).reshape(
            (4, 4)
        )
        self.pose = np.frombuffer(self.pose, dtype=np.float32).reshape((4, 4))
...
class ipc_su(_context_manager):
    ...
    def _download_mesh(self):
        elements_vertices, elements_indices = struct.unpack(
            "<II", self._client.download(2 * _SIZEOF.DWORD, ChunkSize.SINGLE_TRANSFER)
        )
    ...
    def _download_meshes(self):
        return [
            ...
            for _ in range(
                ...
                struct.unpack(
                    "<I",
                    self._client.download(_SIZEOF.DWORD, ChunkSize.SINGLE_TRANSFER),
                )[0],
            )
        ]

    def _download_item(
        self, bi, bk, bo, bp, bl, ba, be, bm, download_meshes, download_collider_meshes
    ):
        ...
        return _su_result(
            ...
            [
                self._download_item(
                    ...
                    bm,
                    task.get_meshes,
                    task.get_collider_meshes,
                )
                for _ in range(0, struct.unpack("<I", header[132:])[0])
            ],
        )
...
class vi_result:
    def __init__(
        self, index, confidence, phrase_duration, phrase_start_time, raw_confidence
    ):
        self.index = index
...
class ipc_vi(_context_manager):
    ...
    def register_commands(self, clear, strings):
        ...
        command.extend(
            struct.pack(
                "<BBB", ipc_vi._CMD_REGISTER_COMMANDS, 1 if (clear) else 0, len(strings)
            )
        )
    ...
    def pop(self):
        ...
        return [
            vi_result(
                ...
                data[(i * 32 + 4) : (i * 32 + 8)],
                data[(i * 32 + 8) : (i * 32 + 16)],
                data[(i * 32 + 16) : (i * 32 + 24)],
                data[(i * 32 + 24) : (i * 32 + 32)],
            )
            for i in range(0, count)
        ]
...
class ipc_umq(_context_manager):
    ...
    def pull_n(self, count):
        return np.frombuffer(
            self._client.download(_SIZEOF.DWORD * count, ChunkSize.SINGLE_TRANSFER),
            dtype=np.uint32,
        )
