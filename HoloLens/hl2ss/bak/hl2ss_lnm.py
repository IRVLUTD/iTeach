...
def get_video_codec_default_bitrate(width, height, framerate, divisor, profile):
    return get_video_codec_bitrate(
        width, height, framerate, divisor, get_video_codec_default_factor(profile)
    )


def get_video_codec_default_options(width, height, framerate, divisor, profile):
    options = dict()
    options[hl2ss.H26xEncoderProperty.CODECAPI_AVEncMPVGOPSize] = (
        get_video_codec_default_gop_size(framerate, divisor)
...
def start_subsystem_pv(
    ...
    hologram_perspective=hl2ss.HologramPerspective.PV,
):
    hl2ss.start_subsystem_pv(
        host,
        port,
        enable_mrc,
...
def rx_rm_vlc(
    host,
    port,
    chunk=hl2ss.ChunkSize.RM_VLC,
    mode=hl2ss.StreamMode.MODE_1,
    divisor=1,
    profile=hl2ss.VideoProfile.H265_MAIN,
    level=hl2ss.H26xLevel.DEFAULT,
    bitrate=None,
    options=None,
    decoded=True,
):
    if bitrate is None:
        bitrate = get_video_codec_default_bitrate(
            hl2ss.Parameters_RM_VLC.WIDTH,
            hl2ss.Parameters_RM_VLC.HEIGHT,
    ...
    if options is None:
        options = get_video_codec_default_options(
            hl2ss.Parameters_RM_VLC.WIDTH,
            hl2ss.Parameters_RM_VLC.HEIGHT,
            hl2ss.Parameters_RM_VLC.FPS,
            divisor,
    ...
    return (
        hl2ss.rx_decoded_rm_vlc(
            host, port, chunk, mode, divisor, profile, level, bitrate, options
        )
        if (decoded)
        else hl2ss.rx_rm_vlc(
            host, port, chunk, mode, divisor, profile, level, bitrate, options
        )
    )


def rx_rm_depth_ahat(
    host,
    port,
    chunk=hl2ss.ChunkSize.RM_DEPTH_AHAT,
    mode=hl2ss.StreamMode.MODE_1,
    divisor=1,
    ...
    if bitrate is None:
        bitrate = get_video_codec_default_bitrate(
            ...
            hl2ss.Parameters_RM_DEPTH_AHAT.FPS,
            divisor,
            profile_ab,
        ) * (
            4
            if (
                (profile_z == hl2ss.DepthProfile.SAME)
                and (profile_ab != hl2ss.VideoProfile.RAW)
            )
            else 1
        )
...
def rx_rm_depth_longthrow(
    ...
    return (
        hl2ss.rx_decoded_rm_depth_longthrow(
            host, port, chunk, mode, divisor, png_filter
        )
        if (decoded)
        else hl2ss.rx_rm_depth_longthrow(host, port, chunk, mode, divisor, png_filter)
    )
...
def rx_pv(
    ...
    if bitrate is None:
        bitrate = get_video_codec_default_bitrate(
            width, height, framerate, divisor, profile
        )
...
def rx_microphone(
    host,
    port,
    chunk=hl2ss.ChunkSize.MICROPHONE,
    profile=hl2ss.AudioProfile.AAC_24000,
    level=hl2ss.AACLevel.L2,
    decoded=True,
):
    return (
        hl2ss.rx_decoded_microphone(host, port, chunk, profile, level)
        if (decoded)
        else hl2ss.rx_microphone(host, port, chunk, profile, level)
    )
