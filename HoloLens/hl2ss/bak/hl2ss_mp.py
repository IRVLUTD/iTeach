...
def _get_nearest_packet(data, timestamp):
    ...
    return (
        l
        if (abs(data[l].timestamp - timestamp) < abs(data[r].timestamp - timestamp))
        else r
    )
...
class _interconnect(mp.Process):
    ...
    def _get_nearest(self, sink_din, sink_dout):
        ...
        response = (
            (None, None)
            if (index is None)
            else (self._frame_stamp - self._buffer.length() + 1 + index, buffer[index])
        )
...
def _create_interface_interconnect():
    return _net_interconnect(
        mp.Queue(), mp.Queue(), mp.Semaphore(_interconnect.IPC_SEMAPHORE_VALUE)
    )
...
class _module:
    def __init__(self, receiver, buffer_size):
        ...
        self._source = _create_source(
            receiver, self._source_wires, self._interconnect_wires
        )
        self._interconnect = _create_interconnect(
            buffer_size, self._source_wires, self._interconnect_wires
        )
...
class consumer:
    ...
    def create_sink(self, producer, port, manager, semaphore):
        sink_semaphore = (
            ...
            else manager.Semaphore(_interconnect.IPC_SEMAPHORE_VALUE)
            if (semaphore is ...)
            else self._sink_semaphore[semaphore]
        )
        sink_wires = _create_interface_sink(
            manager.Queue(), manager.Queue(), sink_semaphore
        )
