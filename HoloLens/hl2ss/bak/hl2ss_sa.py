...
from ..hl2ss import hl2ss, hl2ss_lnm, hl2ss_3dcv
        ...
        for index, mesh in self._ipc.get_meshes(tasks, self._threads).items():
            ...
            rcs.add_triangles(
                o3d.t.geometry.TriangleMesh.from_legacy(
                    sm_mesh_to_open3d_triangle_mesh(mesh)
                )
            )
            surface_info = updated_surfaces[index]
            self._updated_surfaces[surface_info.id] = _sm_manager_entry(
                surface_info.update_time, mesh, rcs
            )
...
class integrator:
    def __init__(
        self, voxel_size=3 / 512, block_resolution=16, block_count=10000, device="cpu:0"
    ):
    ...
    def set_projection(self, projection):
        self._projection32 = o3d.core.Tensor(projection).to(
            self._device, o3d.core.float32
        )
    ...
    def extract_point_cloud(self, weight_threshold=3.0, estimated_point_number=-1):
        return self._vbg.extract_point_cloud(
            float(weight_threshold), int(estimated_point_number)
        )

    def integrate(self):
        frustum_block_coords = self._vbg.compute_unique_block_coordinates(
            self._depth,
            self._intrinsics64,
            self._extrinsics64,
            self._depth_scale,
            self._depth_max,
            self._trunc_voxel_multiplier,
        )
        self._vbg.integrate(
    ...
    def erase_full(self):
        ...
        # o3d.core.cuda.synchronize()
        voxel_coords, voxel_indices = self._vbg.voxel_coordinates_and_flattened_indices(
            buf_indices
        )
        # o3d.core.cuda.synchronize()
    ...
    def update_full(self):
        ...
        mask_inlier = (
            (depth_readings > 0)
            & (sdf >= -self._trunc)
            & (depth_readings < self._depth_max)
        )
