...
import cv2
import av
from ..hl2ss import hl2ss, hl2ss_io, hl2ss_mp, hl2ss_lnm, hl2ss_3dcv
...
class wr_process_producer(mp.Process):
    def __init__(self, filename, producer, port, user):
        super().__init__()
        self._event_stop = mp.Event()
        self._wr = hl2ss_io.create_wr_from_rx(
            filename, producer.get_receiver(port), user
        )
    ...
    def run(self):
        self._frame_stamp = hl2ss_lnm.get_sync_frame_stamp(
            self._sink.get_attach_response() + 1, self._sync_period
        )
...
class microphone_resampler:
    ...
    def resample(self, data, profile):
        in_frame = av.AudioFrame.from_ndarray(
            data,
            format="s16" if (profile == hl2ss.AudioProfile.RAW) else "fltp",
            layout="stereo",
        )
...
def si_unpack_hand(hand):
    poses = [
        hand.get_joint_pose(joint) for joint in range(0, hl2ss.SI_HandJointKind.TOTAL)
    ]
...
def si_ray_transform(ray, transform4x4):
    return np.hstack(
        (
            hl2ss_3dcv.transform(ray[:, 0:3], transform4x4),
            hl2ss_3dcv.orient(ray[:, 3:6], transform4x4),
        )
    )
...
def _create_csv_header_for_si_head_pose():
    return (
        ["head pose valid"]
        + [f"head position {w}" for w in ["x", "y", "z"]]
        + [f"head forward {w}" for w in ["x", "y", "z"]]
        + [f"head up {w}" for w in ["x", "y", "z"]]
    )
...
def _create_csv_header_for_si_eye_ray():
    return (
        ["eye ray valid"]
        + [f"eye ray origin {w}" for w in ["x", "y", "z"]]
        + [f"eye ray direction {w}" for w in ["x", "y", "z"]]
    )
...
def _create_csv_header_for_si_hand_joint(hand_name, joint_kind):
    ...
    return (
        [f"{hand_name} {joint_name} position {u}" for u in ["x", "y", "z"]]
        + [f"{hand_name} {joint_name} orientation {u}" for u in ["x", "y", "z", "w"]]
        + [f"{hand_name} {joint_name} radius"]
        + [f"{hand_name} {joint_name} accuracy"]
    )
...
def _create_csv_header_for_si_payload():
    return (
        _create_csv_header_for_si_head_pose()
        + _create_csv_header_for_si_eye_ray()
        + _create_csv_header_for_si_hand("left")
        + _create_csv_header_for_si_hand("right")
    )
...
def _create_csv_header_for_eet_ray(ray_name):
    return (
        [f"{ray_name} valid"]
        + [f"{ray_name} origin {w}" for w in ["x", "y", "z"]]
        + [f"{ray_name} direction {w}" for w in ["x", "y", "z"]]
    )
...
def _create_csv_header_for_eet_payload():
    return (
        _create_csv_header_for_eet_calibration()
        + _create_csv_header_for_eet_ray("combined")
        + _create_csv_header_for_eet_ray("left")
        + _create_csv_header_for_eet_ray("right")
        + _create_csv_header_for_eet_field("left openness")
        + _create_csv_header_for_eet_field("right openness")
        + _create_csv_header_for_eet_field("vergence distance")
    )
...
def _create_csv_header_for_rm_imu(port):
    return (
        _create_csv_header_for_timestamp()
        + _create_csv_header_for_rm_imu_payload(port)
        + _create_csv_header_for_pose()
    )
...
def _create_csv_header_for_pv():
    return (
        _create_csv_header_for_timestamp()
        + _create_csv_header_for_pv_payload()
        + _create_csv_header_for_pose()
    )
...
def _create_csv_header_for_eet():
    return (
        _create_csv_header_for_timestamp()
        + _create_csv_header_for_eet_payload()
        + _create_csv_header_for_pose()
    )
...
def _create_csv_row_for_rm_imu_frame(frame):
    return [
        str(frame.vinyl_hup_ticks),
        str(frame.soc_ticks),
        str(frame.x),
        str(frame.y),
        str(frame.z),
        str(frame.temperature),
    ]
...
def _create_csv_row_for_pv_payload(payload):
    return (
        payload.focal_length.astype(str).tolist()
        + payload.principal_point.astype(str).tolist()
    )
...
def _create_csv_row_for_si_head_pose(valid, pose):
    return (
        ...
        + pose.position.astype(str).tolist()
        + pose.forward.astype(str).tolist()
        + pose.up.astype(str).tolist()
    )
...
def _create_csv_row_for_si_eye_ray(valid, ray):
    return (
        valid.astype(str).tolist()
        + ray.origin.astype(str).tolist()
        + ray.direction.astype(str).tolist()
    )
...
def _create_csv_row_for_si_hand_joint(pose):
    return (
        pose.position.astype(str).tolist()
        + pose.orientation.astype(str).tolist()
        + pose.radius.astype(str).tolist()
        + pose.accuracy.astype(str).tolist()
    )
...
def _create_csv_row_for_si_payload(payload):
    return (
        _create_csv_row_for_si_head_pose(
            payload.is_valid_head_pose(), payload.get_head_pose()
        )
        + _create_csv_row_for_si_eye_ray(
            payload.is_valid_eye_ray(), payload.get_eye_ray()
        )
        + _create_csv_row_for_si_hand(
            payload.is_valid_hand_left(), payload.get_hand_left()
        )
        + _create_csv_row_for_si_hand(
            payload.is_valid_hand_right(), payload.get_hand_right()
        )
    )
...
def _create_csv_row_for_eet_payload(payload):
    return (
        _create_csv_row_for_eet_calibration(payload.calibration_valid)
        + _create_csv_row_for_eet_ray(payload.combined_ray_valid, payload.combined_ray)
        + _create_csv_row_for_eet_ray(payload.left_ray_valid, payload.left_ray)
        + _create_csv_row_for_eet_ray(payload.right_ray_valid, payload.right_ray)
        + _create_csv_row_for_eet_field(
            payload.left_openness_valid, payload.left_openness
        )
        + _create_csv_row_for_eet_field(
            payload.right_openness_valid, payload.right_openness
        )
        + _create_csv_row_for_eet_field(
            payload.vergence_distance_valid, payload.vergence_distance
        )
    )


def _create_csv_row_for_rm_vlc(data):
    return _create_csv_row_for_timestamp(data.timestamp) + _create_csv_row_for_pose(
        data.pose
    )
...
def _create_csv_row_for_rm_imu(data):
    return (
        _create_csv_row_for_timestamp(data.timestamp)
        + _create_csv_row_for_rm_imu_payload(data.payload)
        + _create_csv_row_for_pose(data.pose)
    )
...
def _create_csv_row_for_pv(data):
    return (
        _create_csv_row_for_timestamp(data.timestamp)
        + _create_csv_row_for_pv_payload(data.payload)
        + _create_csv_row_for_pose(data.pose)
    )
...
def _create_csv_row_for_si(data):
    return _create_csv_row_for_timestamp(
        data.timestamp
    ) + _create_csv_row_for_si_payload(data.payload)


def _create_csv_row_for_eet(data):
    return (
        _create_csv_row_for_timestamp(data.timestamp)
        + _create_csv_row_for_eet_payload(data.payload)
        + _create_csv_row_for_pose(data.pose)
    )
...
def unpack_to_mp4(input_filenames, output_filename):
    ...
    readers = [
        hl2ss_io.create_rd(input_filename, hl2ss.ChunkSize.SINGLE_TRANSFER, None)
    ...
    streams = [
        container.add_stream(
            ...
            rate=get_av_framerate(reader.port)
            if (get_av_framerate(reader.port) is not None)
            else reader.framerate,
        )
