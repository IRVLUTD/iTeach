...
public class DrawRectangle : MonoBehaviour, IMixedRealityPointerHandler
{
    ...
    //Handler for drawing either a door or a handle
    enum Drawmode
    {
        none,
        handle,
    ...
    public void OnPointerUp(MixedRealityPointerEventData eventData)
    {
        if (isDrawing)
        {
            ...
            switch(mode){
                case Drawmode.door:
                    currentRectangle.GetComponent<BoxIdentifier>().SetBoxType(BoxIdentifier.RectType.door);
    ...
    public string getLabels(){
        ...
        for(int i=0; i<rects.Count; i++){
            ...
            string type = Helpers.RectTypeToLabel(((GameObject)rects[i]).GetComponent<BoxIdentifier>().GetBoxType());
