...
class HoloRosPublisher:
    ...
    def save_image(self, image, timestamp):
        ...
        save_directory = "HoloLensPOVFrames"  # Change this to your desired directory
        os.makedirs(save_directory, exist_ok=True)

        # Convert rospy.Time to microseconds
        timestamp_microsec = int(timestamp.to_sec() * 1e6)

        # Create a filename with the timestamp in microseconds
        filename = f"hl_pov_{timestamp_microsec}.jpg"
    ...
    def _pv_publisher(self, enable_mrc=True):
        prev_timestamp = 0
        save_interval = 1 / 15.0  # Time interval for 15 fps
        ...
        if self._enable_pv:
            ...
            last_save_time = rospy.get_time()  # Track the last save time
            ...
            while not rospy.is_shutdown():
                ...
                # Save image at 15 fps
                current_time = rospy.get_time()
                if current_time - last_save_time >= save_interval:
                    # self.save_image(image, msgTimestamp)
